...
def main() -> None:
    ...
    # 7. Calcul du MSE par simulation (en espace normalisé, c'est suffisant pour classer)
    mse = mse_per_simulation(preds_norm, y_test_norm)  # (N,)

    best_idx = mse.argmin().item()
    worst_idx = mse.argmax().item()

    print("\n─── Résultats ───────────────────────────────────────")
    for label, idx in [("MEILLEURE", best_idx), ("PIRE", worst_idx)]:
        sim_id = sim_ids_test[idx].item()
        print(
            f"  {label} prédiction : index={idx}, sim_id={sim_id}, MSE={mse[idx]:.6f}"
        )
    print("─────────────────────────────────────────────────────\n")

    # 8. Dénormalisation et affichage
    for label, idx in [("MEILLEURE", best_idx), ("PIRE", worst_idx)]:
        sim_id = sim_ids_test[idx].item()
